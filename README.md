Stable Diffusion 모델의 공개 이후, Diffusion 기반 모델이 굉장히 많이 발표되는 것 같다.  
유저가 만들고 싶은 이미지에 대한 설명을 text prompt로 활용하여 유저의 의도를 전달해줄 수 있다는 것이 Diffusion 기반 모델의 가장 큰 강점이라고 생각한다.  
...
1. [데이터셋 준비](#데이터셋-준비)
2. [GAN 모델 학습](#gan-모델-학습)
3. [GAN 모델 추론](#gan-모델-추론)
4. [Diffusion 모델 학습](#diffusion-모델-학습)
5. [Diffusion 모델 추론](#diffusion-모델-추론)
6. [결과 비교](#결과-비교)
7. [결론](#결론)
...
단일 모델으로 개와 고양이를 conditional하게 생성할 수 있도록 학습 진행.
